...
export async function resetDomainTables(pool: Pool): Promise<void> {
  ...
  await pool.query(`
    TRUNCATE TABLE payment_reconciliation_discrepancies, payment_reconciliation_runs,
                   dead_letter_events, notification_jobs, analytics_events,
                   safety_risk_assessments, safety_events, location_events, safety_sessions,
                   booking_match_results, matching_runs,
