...
**AI**
- LLM/NLP'nin görevi *"müşteri ne istiyor?"*. Provider seçimi **kesinlikle** LLM'e bırakılmaz.
...
| 1 | Foundation: monorepo, NestJS, FastAPI, Postgres+PostGIS, Redis, Docker, CI, migration, health | sırada |
| 2 | Core backend: auth, users, roles, RBAC, profiller, service catalog, skills, error handling + **audit (rol ayrımı/hash zinciri), outbox, idempotency altyapısı** | |
| 3 | Identity: adapter, identity_records, unique identity, recovery, verification levels | |
| 4 | Provider & Booking: availability, PostGIS service areas, booking state machine | |
| 5 | Payment & Digital Proof: adapter, webhook idempotency, documents, disputes | |
...
Faz 1'de doldurulacak (henüz build sistemi kurulmadı). Yer tutucu:

```bash
# local altyapı            : docker compose -f infra/docker/docker-compose.yml up -d
# core backend             : npm run dev --workspace=services/api
# migration                : npm run migration:run --workspace=services/api
# ai servisi               : uv run fastapi dev services/ai/app/main.py
# tüm testler              : npm test && (cd services/ai && uv run pytest)
```
...
| `docs/architecture/adr/` | Architecture Decision Record'lar (0001-0013) |
