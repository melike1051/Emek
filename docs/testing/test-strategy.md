...
| T-05  | Concurrent booking — aynı provider/aynı zaman aralığı için N eşzamanlı istek                                          | tek booking oluşur, diğerleri `BOOKING_CONFLICT`; EXCLUDE constraint kanıtlanır; **5xx üretilmez** — `40P01` deadlock'u yeniden denenir, 500'e dönüşmez (R-97) | 4                                                                                                                                                                                                      |
...
| T-11  | Dispute/`SAFETY_HOLD` varken release denemesi                                                                         | bloklanır, gerekçe audit'li                                                                                                                                    | ✅ Faz 5 (dispute + SAFETY_HOLD); ✅ Faz 8: panik aynı transaction'da `SAFETY_HOLD` uygular (`safety.integration`)                                                                                     |
...
| T-20  | Safety panic — ML, matching ve routing servisleri down                                                                | panic kaydedilir, `EMERGENCY` atanır, alert yayınlanır, p95 hedefi tutar                                                                                       | 8 ✅ `safety.integration` (panik: AI erişilemez, anomali çağrısı 0; bildirim hatasında kalıcı) + EXP-004 latency                                                                                       |
| T-21  | Geofence ihlali / rota sapması                                                                                        | ilgili `safety_event` üretilir, risk seviyesi yükselir                                                                                                         | 8 ✅ `geofence.spec`, `safety-rules.spec`, `safety.integration` (debounce, jitter, giriş/çıkış olayı)                                                                                                  |
| T-22  | Anomaly false positive                                                                                                | `WARNING` seviyesinde doğrulama istenir, otomatik cezai aksiyon alınmaz; FPR ölçülür                                                                           | 8 ✅ `risk-aggregation.spec` (ML ≤ WARNING), `safety.integration`, FPR EXP-004'te ölçüldü                                                                                                              |
| T-23  | Oturum kapalıyken gelen telemetri                                                                                     | reddedilir (`SAFETY_SESSION_NOT_ACTIVE` / `SAFETY_SESSION_ALREADY_CLOSED`), veri yazılmaz                                                                      | 8 ✅ `safety.integration`                                                                                                                                                                              |
| T-24  | Location retention                                                                                                    | süresi geçen yüksek frekanslı kayıtlar gerçekten silinir/agregatlanır                                                                                          | 8 ✅ `safety.integration` (silme, yuvarlama, açık oturum korunur) · 12 ✅ `security.integration` (hesap anonimleştirme, event/DLQ/analytics silme, idempotent tarama); partition düşürme Faz 14 (R-69) |
...
| T-33  | **Sahte/replay/geri tarihli telemetri** — mock-location, tekrar gönderilen paket, ileri/geri kaydırılmış `capturedAt` | örnek bazında ret nedeni (paket 200); sunucu zamanı yetkili, sıra numarası monoton, mock-location sinyali kayda geçer ve kurala (R06) girer                    | 8 ✅ `telemetry-validator.spec`, `safety.integration`                                                                                                                                                  |
...
| T-36  | Audit hash zinciri kopukluğu                                                                                          | doğrulama işi kopukluğu tespit eder ve alarm üretir                                                                                                            | 12 ✅ `security.integration` (değiştirilmiş satır bulunur, doğrulama kayıtları değiştirmez, kopukluktan sonra ilerlemez); alarm kuralı Faz 13                                                          |
...
- Güvenlik izleyicisi (değerlendirme, süre aşımı, retention zamanlayıcısı) integration
  testlerinde kapalıdır (`SAFETY_MONITOR_ENABLED=false`); testler aynı işleri servis
  üzerinden açıkça çağırır. Zamanlayıcının test verisini kendi başına değiştirmesi,
  testin neyi ölçtüğünü belirsizleştirirdi (Faz 8).
- Safety testlerinde değiştirilen tek şey **dış sınırlardır** (anomali istemcisi,
  acil durum bildirimi). "AI down" senaryosu gerçek HTTP istemcisinin erişilemeyen bir
  adrese bağlanmasıyla ölçülür. Test telemetrisi fiziksel olarak mümkün olmalıdır:
  6 saniyede 1 km'lik sıçrama doğrulayıcı tarafından (doğru biçimde) reddedilir.

## Yük testi aracı kararı (Faz 14)

Yukarıdaki tabloda yük satırı başlangıçta **k6/Locust** öngörüyordu. Faz 14'te bu
**kullanılmadı**; karar ve gerekçesi:

- k6/Locust yerel ortamda kurulu değil ve yeni bir runtime/binary bağımlılığı olurdu
  (CLAUDE.md §5: "gereksiz dependency eklenmez").
- Repoda aynı işi yapan çalışan bir desen zaten vardı (`scripts/exp-004-latency.ts`):
  gerçek Nest uygulaması, gerçek Postgres/Redis, deterministik fixture, JSON çıktı.
  Genişletildi → `scripts/perf-booking-load.ts` (`npm run perf:booking`).
- HTTP dışı ölçümler (havuz bekleme kuyruğu, outbox satırları, overbooking sayımı)
  zaten süreç içi erişim gerektiriyor; dış bir yük aracı bunları göremezdi.

**Bu kararın bedeli** (EXP-007 §8'de limitation olarak kayıtlı): dış istemci olmadığı
için gerçek ağ, TLS ve load balancer katmanı ölçülmez; yük istemcisi uygulamayla aynı
makineyi ve CPU'yu paylaşır. Gerçek ortam (Faz 13, R-93) açıldığında dışarıdan yük
üreten bir koşu ayrıca gerekir.

## Arıza ve kurtarma testleri (Faz 14)

"Mutlu yol + tek seferlik hata" zaten kapsanıyordu. Faz 14'te eklenen soru farklı:
**süreç ortadan kaybolursa** ne oluyor? Üç kurtarma mekanizması ayrı ayrı sınanır
(`test/failure-recovery.integration.spec.ts` + `analytics.integration.spec.ts`):

| Mekanizma            | Sınanan                                                                              |
| -------------------- | ------------------------------------------------------------------------------------ |
| Outbox kiralaması    | Kirada olan event ikinci instance tarafından alınamaz; tam olarak bir yayın          |
| Outbox kurtarma      | Kira dolunca yeniden sahiplenilir — çöken instance event'i götürmez                  |
| Outbox deneme sınırı | `FAILED` kayıt kendiliğinden yeniden denenmez (kurtarma operasyonel karardır)        |
| Consumer geçici hata | NACK + dedup işareti **geri alınır** → yeniden teslim başarılı, yan etki tam bir kez |
| Consumer kalıcı hata | ACK + DLQ; işaret **kalır** → yeniden teslim ikinci DLQ kaydı/yan etki üretmez       |
| Worker restart       | BigQuery export ve mutabakat worker'ları yeniden başladığında çift iş üretmez        |

İki bağlayıcı kural:

1. **Zaman beklenmez.** Kira ve backoff pencereleri SQL ile geriye alınır. Bekleyen
   bir test, ölçtüğü davranışı değil makinenin hızını ölçer.
2. **Bileşen değil dünya değiştirilir.** Bozulma, servisi mock'layarak değil
   erişilemez hâle getirerek (erişilemez port, yanıtlamayan TCP dinleyicisi, dış
   servis sınırındaki taşıma) üretilir; böylece ölçülen şey gerçek hata yoludur.

**Kapsamın sınırı (Faz 14 kapanışı, R-98).** Yukarıdaki mekanizmaların tamamı
"Postgres ayakta" varsayar. **PostgreSQL'in kendi kesintisi ve geri gelmesi** (bağlantı
havuzu kurtarması, yeniden bağlanma, yarıda kalan transaction'ların akıbeti) hiç
sınanmadı; bağımlılık bozulma kollarında da Postgres yoktur. İkinci boşluk, **asılı
kalan** kimlik sağlayıcısıdır: ölçülen kol mock'un hızlı hatasıdır, oysa Faz 14'ün
kendi bulgusu asılı kalan bağımlılığın hata verenden çok daha pahalı olduğudur. İkisi de
ölçülebilir; ölçülmemeleri bir kapsam kararıdır.

**Paket birlikte koşmak zorundadır.** Slot deadlock'u (R-97), `booking-concurrency`
tek başına koşarken **hiç görünmedi**; yalnızca tüm integration paketleri birlikte
koşarken, dört koşunun ikisinde ortaya çıktı. Bir eşzamanlılık testinin izole hâlde
geçmesi kanıt değildir: zamanlama penceresini büyüten şey diğer paketlerin yüküdür.
Bu yüzden eşzamanlılık testleri **yalıtılmış çalıştırmayla** yeşil sayılmaz ve
aralıklı bir düşüş "flaky" diye geçiştirilmez — önce yığın izi okunur.

Ayrıca `audit_logs` **hiçbir zaman truncate edilmez** (append-only, KVKK/denetim).
Audit iddiaları bu yüzden tüm tabloya değil, `currentAuditMaxId`/`auditActionsSince`
ile **o teste** bağlanır; aksi hâlde aynı veritabanında ikinci koşuda düşerler.

## Yük ölçümünde metrik tuzakları (Faz 14 dersi)

Ölçüm sırasında iki metriğin **yanıltıcı** olduğu görüldü; raporlamada ikisi de ayrı
ayrı verilir:

1. **Ham RPS**, yüksek eşzamanlılıkta çoğunlukla ucuz 429 reddini sayar — işin hızını
   değil reddetme hızını ölçer. Bu yüzden `created_per_second` (etkin rezervasyon
   üretimi) ayrıca raporlanır.
2. **Toplam p50/p95**, aynı ucuz 429'lar yüzünden aşağı çekilir. Bu yüzden yalnızca
   201 dönen isteklerin yüzdelikleri (`created_p50_ms`/`created_p95_ms`) ayrıca
   raporlanır.

Kural: "iyi görünen" toplulaştırma seçilmez; hangi isteklerin sayıldığı her zaman
açıkça yazılır.
