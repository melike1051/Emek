...
| T-09  | Payment webhook duplication — aynı `external_event_id` iki kez                                                        | ikinci event yan etki üretmez, para iki kez serbest bırakılmaz, 200 döner                                                     | 5         |
| T-10  | Out-of-order payment event                                                                                            | geri durum geçişi reddedilir, durum monotonluğu korunur                                                                       | 5         |
| T-11  | Dispute/`SAFETY_HOLD` varken release denemesi                                                                         | bloklanır, gerekçe audit'li                                                                                                   | 5, 8      |
| T-12  | Storage erişimi                                                                                                       | nesneler private; yalnızca kısa ömürlü signed URL ile erişilebilir; imza süresi dolmuş URL reddedilir                         | 5         |
...
| T-34  | Ödeme yetkilendirme süresi dolmuş, release deneniyor                                                                  | `PAYMENT_AUTHORIZATION_EXPIRED`; re-authorization akışı tetiklenir, çift yetkilendirme oluşmaz                                | 5         |
| T-35  | Uygulama DB rolüyle `audit_logs` UPDATE/DELETE denemesi                                                               | yetki hatası — rol ayrımı Faz 2'de kurulu                                                                                     | 2         |
...
| T-38  | Event'ten para hareketi tetiklenmesi                                                                                  | duplicate `ProviderAccepted`/`ServiceCompleted` teslimi PSP'ye ikinci `authorize`/`capture` göndermez                         | 9         |
