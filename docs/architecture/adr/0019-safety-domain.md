...
replay'i engeller; ret de sıra numarasını tüketir. Telemetri ucunun oran sınırı
**fail-open**'dır (tek istisna): Redis kesintisinde telemetriyi reddetmek her oturumda
sahte "telemetri kesildi" alarmı üretirdi; kalıcı koruma oturumdaki sıra/aralık
kontrolüdür.
...
### 6. Risk toplama politikası (`risk-agg-v1`)
...
3. Anomali skoru tek başına en fazla `WARNING`; bir kural uyarısıyla birlikte ikinci
   kanıttır. Kalitesi < 0.5 olan skor hiç sayılmaz.
...
oturum geçmişi); bu açıkça belgelenir (R-61).
...
kesildiğinde de değerlendirmeyi sürdürür — boşluğu fark ettiren şey zamandır.

### 9. Panik yalnızca PostgreSQL'e bağlıdır

Olay + oturum (`EMERGENCY`) + rezervasyon askısı (`SAFETY_HOLD` → ödeme donar) + audit +
outbox (`SafetyAlertRaised`) **tek transaction**. Oran sınırı guard'ı (Redis) takılmaz;
...
durum makinesi reddederse panik **yine kaydedilir**. Etkin panik varken tekrar basış
yan etki üretmez; operatör çözdükten sonra yeni panik kabul edilir. Bildirim commit
...
Ham konum `retention_expires_at` (planlanan bitiş + 30 gün, panikte kanıt süresi) sonra
silinir; son konum silinir, hizmet noktası ~1 km'ye yuvarlanır. Olaylar ve
...
- (−) Eşzamanlı paniklerin gecikmesi global audit kilidiyle (ADR-0013) seri hâle
  geliyor olabilir (R-54, ölçüldü, nedeni hipotez).
